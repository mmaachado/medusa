...
def progress_bar(iterable: typing.Any, prefix: str = 'progress:', suffix: str = 'complete', decimals: int = 1, length: int = 100, fill: str = '█', print_end: str = "\r") -> str:
    r"""
    Output to user a progress bar of the module execution. 

    This function will output on the users terminal an progress bar dynamically
    about the module execution status.
    ...
    suffix : str
        Suffix string.
    decimals : int
        Positive numbers of decimals in percent complete.
    length : int
    ...
    -------
    str
        Return a progress bar in user terminal showing dynamically the status 
        of the module execution.
    ...
    def print_progress_bar(iteration) -> str:
...
def linux_socket_status() -> dict:
    """
    Linux socket status codes.
    """
